...
int main()
{
    ...
    for (double i = a; i <= b; i += (b - a) / (1e5-N))
    {        
    ...
    {
        cx[i] = ((b + a) / 2) + ((b - a) / 2) * (cos(M_PI * (2 * i + 1) / (2 * N0)));//2*N0=15
        cy[i] = func1(cx[i]);
    ...
    for (int i = 0; i < N0; i++)//тут включительно или нет если точек по чебышеву 15 всего???
    {
    ...
    for (double i = a; i <= b; i += (b - a) / (1e5 - N))
    {
        dataFile << abs(func1(i) - Newton(x, i, N0)) << std::endl;
